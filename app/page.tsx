...
export default function Home() {
  ...
  const clearFilter = () => {
    setFrom(''); setTo(''); setSelectedTagIds([]); setQ('')
  }

  return (
    <main className="min-h-screen bg-[#071210] text-[#d4e8d8]">
      <div className="max-w-4xl mx-auto px-4 py-8">

        {/* ヘッダー */}
        <header className="flex items-center gap-2.5 mb-8">
          <Sprout size={20} className="text-[#b5e48c]" />
          <h1 className="text-base font-semibold text-[#f0f7f1] tracking-widest uppercase">
            sns memo
          </h1>
        </header>
        ...
        {/* フィルター */}
        <div className="mb-6 p-4 rounded-xl border border-[#1a3a25] bg-[#0a1a10]">
          <FilterBar
            from={from} to={to}
            selectedTagIds={selectedTagIds} q={q}
            allTags={allTags}
            onFromChange={setFrom} onToChange={setTo}
            onTagToggle={toggleTag} onQChange={setQ}
            onClear={clearFilter}
          />
        </div>

        {/* メモ件数 */}
        <div className="mb-4 text-xs text-[#3a5a45] tracking-wider">
          {loading ? '読み込み中…' : `${notes.length} 件`}
        </div>

        {/* メモグリッド */}
        {!loading && notes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-[#2a4a35]">
